...
void Memory::load_program(const std::vector<uint8_t>& program, uint16_t start_address) {
...
void Memory::dump_memory(uint16_t start, uint16_t length) {
    std::cout << "Memory dump from 0x" << std::hex << std::setw(4) << std::setfill('0') 
              << start << " to 0x" << (start + length - 1) << ":\n";
