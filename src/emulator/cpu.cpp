...
CPU::CPU() : halted_(false), debug_mode_(false) { reset(); }
...
void CPU::load_program(const std::vector<uint8_t> &program,
  ...
  if (debug_mode_) {
    std::cout << "Program loaded at 0x" << std::hex << start_address
              << ", size: " << std::dec << program.size() << " bytes"
              << std::endl;
  }
}
...
void CPU::run() {
  ...
  if (debug_mode_) {
    std::cout << "CPU execution stopped after " << cycle_count
              << " cycles. Halted: " << (halted_ ? "Yes" : "No") << std::endl;
    if (cycle_count >= MAX_CYCLES) {
      std::cout << "Warning: Execution stopped due to cycle limit (possible "
                   "infinite loop)"
                << std::endl;
    }
  }
...
bool CPU::step() {
  ...
  try {
    ...
    if (tracer_) {
      ...
      memory_.set_trace_callback(
          [this](uint16_t addr, uint8_t oldv, uint8_t newv) {
            MemWriteEvent ev{addr, oldv, newv};
            if (tracer_)
              tracer_->record_mem_write(ev);
          });
    }
...
void CPU::execute_arithmetic(const DecodedInstruction &instr,
                             ALU::Operation op) {
...
uint16_t CPU::resolve_operand(const DecodedInstruction &instr,
                              bool is_destination) {
  switch (instr.mode) {
  ...
  case AddressingMode::PC_RELATIVE: {
    uint16_t address =
        registers_.get_pc() + static_cast<int16_t>(instr.extra_word);
    if (is_destination) {
      memory_.write_word(address, registers_.get_gpr(instr.rd));
      return 0;
    }
...
void CPU::print_instruction(const DecodedInstruction &instr) const {
  std::cout << "PC: 0x" << std::hex << std::setw(4) << std::setfill('0')
            ...
            << " (mode: " << mode_to_string(instr.mode) << ")" << std::dec
            << std::endl;
}
...
bool CPU::check_condition(Opcode opcode) {
  switch (opcode) {
  case Opcode::JZ:
    return registers_.is_zero();
  case Opcode::JNZ:
    return !registers_.is_zero();
  case Opcode::JC:
    return registers_.is_carry();
  case Opcode::JNC:
    return !registers_.is_carry();
  case Opcode::JN:
    return registers_.is_negative();
  default:
    return false;
  }
}