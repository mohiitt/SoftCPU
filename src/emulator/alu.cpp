...
uint16_t ALU::execute(Operation op, uint16_t operand_a, uint16_t operand_b, Registers& registers) {
    uint16_t result = 0;
...
uint16_t ALU::add(uint16_t a, uint16_t b) {
    return static_cast<uint16_t>((static_cast<uint32_t>(a) + static_cast<uint32_t>(b)) & 0xFFFF);
}
...
uint16_t ALU::bitwise_and(uint16_t a, uint16_t b) {
    return a & b;
}

uint16_t ALU::bitwise_or(uint16_t a, uint16_t b) {
    return a | b;
}

uint16_t ALU::bitwise_xor(uint16_t a, uint16_t b) {
    return a ^ b;
}
...
void ALU::update_flags_arithmetic(uint16_t result, uint16_t operand_a, uint16_t operand_b, 
    ...
    // Overflow flag
    bool overflow = is_subtraction ? calculate_overflow_sub(operand_a, operand_b, result)
...
void ALU::update_flags_shift(uint16_t result, bool carry_out, Registers& registers) {
    // Zero flag
    registers.set_flag(Registers::FLAG_Z, calculate_zero_flag(result));
...
bool ALU::calculate_zero_flag(uint16_t result) {
    return result == 0;
}
...
bool ALU::calculate_carry_sub(uint16_t a, uint16_t b) {
    return a >= b;  // No borrow needed if a >= b
}
